...
import { addVehicle, selectAddVehicleFeedback, selectUpdateVehicleFeedback, updateVehicle } from '../../../../redux/vehiclesSlice'
...
function Edit(props: Props) { 
  const { onClose, ...vehicle_details } = props
  const toast = useToast({
      position: "top"
  })
  const  { data: stations, isLoading } = useGetStationsQuery(null)
  const dispatch = useAppDispatch()
  const feedback = useAppSelector(selectUpdateVehicleFeedback)
  const [{ data , errors }, setInputState] = useState<{
    ...
    data: {
        ...
        year: Number(vehicle_details?.year ?? 0) ?? new Date().getFullYear(),
        color: vehicle_details?.color,
        seats: vehicle_details?.seats ?? 0,
        transmission: vehicle_details?.transmission ?? "",
        station_id: vehicle_details?.station_id ?? "",
        hourly_rate: vehicle_details?.hourly_rate ?? 0,
        plate: vehicle_details?.plate ?? "",
        pictures: vehicle_details?.pictures ?? []
    },
    errors: null
  })
