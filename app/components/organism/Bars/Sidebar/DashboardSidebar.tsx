...
function DashboardSidebar() {
  ...
  return (
    <Grid overflowY="scroll"  bg="white" h="full" rowGap="5" position="fixed" w="300px" templateColumns={"1fr"} paddingLeft="10px" paddingRight={"15px"} paddingTop="20px"  templateRows="repeat(13, 47px)" >
        <Flex w="full" {...FlexRowCenterCenter} >
            <Logo/>
        </Flex>
        {
            dashboardRoutes?.filter(({admin})=>!admin)?.map(({name, icon: Icon, link}, index)=>(
                <DashboardSidebarButton onClick={()=>{
                    dispatchAction(setActiveTab(link))
                }} key={index} icon={Icon && <Icon color={activeTab === link ? "primary.900" : "black"}  />} link={link} isActive={!link ? false : activeTab === link}  >
                    {name}
